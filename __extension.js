...
function activate(context) {
  commandDisposable = vscode.commands.registerCommand(
    "poor-mans-t-sql-formatter.formatSql",
    () => {
      ...
      let inputSql = useSelection
        ? doc.getText(editor.selection)
        : doc.getText();

      let oldOffset = doc.offsetAt(selectionActivePoint);
      let oldOffsetPercent =
        inputSql.length == 0 ? 0 : oldOffset / inputSql.length;
      ...
      return new Promise((resolve, reject) => {
        if (
          ...
          vscode.window
            ...
            .then((selectedAction) => {
              if (selectedAction === "Format Anyway") resolve(true);
              else resolve(false);
            });
        else resolve(true);
      })
        .then((formatDespiteMismatch) => {
          if (!formatDespiteMismatch) return;

          libResult = sqlFormatter.formatSql(
            inputSql,
            collectOptions(extensionSettingsObject)
          );

          return new Promise((resolve, reject) => {
            if (extensionSettingsObject.confirmOnError && libResult.errorFound)
              vscode.window
                .showWarningMessage(
            ...
            if (formatDespiteError) {
              editor
                .edit((mutator) => {
                  mutator.replace(
                    useSelection ? editor.selection : getWholeDocRange(doc),
                    libResult.text
                  );
                })
                .then((editCompleted) => {
  ...
  vscode.languages.registerDocumentFormattingEditProvider("sql", {
    provideDocumentFormattingEdits(document, range, options) {
      const inputSql = document.getText();
      const config = getConfig(options);
      const formattedText = sqlFormatter.formatSql(
        inputSql,
        collectOptions(extensionSettingsObject)
      );
    },
